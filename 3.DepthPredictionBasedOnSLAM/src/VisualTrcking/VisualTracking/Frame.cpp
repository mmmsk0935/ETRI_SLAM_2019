...
CFrame::CFrame(CFrame& frame)
	: m_cvColorImage(frame.m_cvColorImage), m_cvDepthImage(frame.m_cvDepthImage)
	, m_vSiftKeyPoints(frame.m_vSiftKeyPoints), m_vSiftDescriptors(frame.m_vSiftDescriptors)
	, m_vORBKeyPoints(frame.m_vORBKeyPoints), m_cvORBDescriptors(frame.m_cvORBDescriptors)
	, R(frame.R), t(frame.t)
...
void	CFrame::SetImage(const cv::Mat& cvColorImage, const cv::Mat& cvDepthImage)
{
	m_cvColorImage = cvColorImage.clone();
	...
	if (vSLAMConfigure.m_nPointFeatureType == PointFeatureType::SIFT_GPU) {
		for (int i = 0; i < (int)m_vSiftKeyPoints.size(); i++) {
			...
			if (nX1 < 10 || nX2 >= m_cvColorImage.cols - 10 || nY1 < 10 || nY2 >= m_cvColorImage.rows - 10)
				continue;
			double dbDepth1 = m_cvDepthImage.at<float>(nY1, nX1) / 1000;
			double dbDepth2 = m_cvDepthImage.at<float>(nY1, nX2) / 1000;
			double dbDepth3 = m_cvDepthImage.at<float>(nY2, nX1) / 1000;
			double dbDepth4 = m_cvDepthImage.at<float>(nY2, nX2) / 1000;
