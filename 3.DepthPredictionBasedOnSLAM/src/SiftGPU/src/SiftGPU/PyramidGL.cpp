...
void PyramidNaive::GenerateFeatureList()
{
	...
	//for (int i = 0, idx = 0; i < _octave_num; GOTO_NEXT_OCTAVE(i, 1))
	//FOR_EACH_OCTAVE(i, reverse)
#ifdef SIFTGPU_ENABLE_REVERSE_ORDERDEBUG
	for (int i = (reverse? _octave_num - 1: 0); (reverse? (i >= 0) : (i < _octave_num)); (reverse? --i : ++i))
#else
	for (int i = 0; i < _octave_num; ++i)
#endif // SIFTGPU_ENABLE_REVERSE_ORDERDEBUG
	{
		//output
		if(GlobalUtil::_timingO)
		{
		...
		for (int j = (reverse ? param._dog_level_num - 1 : 0); (reverse ? (j >= 0) : (j < param._dog_level_num)); (reverse ? (--j) : (++j)))
#else
		for(int j = 0; j < param._dog_level_num; ++j)
#endif // SIFTGPU_ENABLE_REVERSE_ORDER
        {
