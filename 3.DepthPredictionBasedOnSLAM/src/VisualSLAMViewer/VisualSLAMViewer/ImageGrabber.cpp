...
void	CImageGrabber::Initialize(int nCameraType/* = CameraType::Monocular*/, int nVideoCaptureType/* = VideoCaptureType::ImageSet*/)
{
	m_nCameraType = nCameraType;
	m_nVideoCaptureType = nVideoCaptureType;

	if (m_nCameraType == CameraType::Monocular && m_nVideoCaptureType == VideoCaptureType::OnTheFly) {
		m_pVideoCapture = new cv::VideoCapture(0);
		if (m_pVideoCapture->isOpened())
			std::cout << "webcam is connected..." << std::endl;
	}
