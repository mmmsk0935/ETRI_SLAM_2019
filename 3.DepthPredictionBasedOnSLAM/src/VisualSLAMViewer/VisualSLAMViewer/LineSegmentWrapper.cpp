...
{
	m_pLSD = cv::line_descriptor::LSDDetector::createLSDDetector();
	m_pLSDDescriptors = cv::line_descriptor::BinaryDescriptor::createBinaryDescriptor();
	m_pLineSegmentMatcher = cv::line_descriptor::BinaryDescriptorMatcher::createBinaryDescriptorMatcher();
}
...
void	CLineSegmentWrapper::ExtractLineSegment(const cv::Mat& cvColorImage, std::vector<cv::line_descriptor::KeyLine>& vKeyLines, cv::Mat& cvKeyLineDescriptors)
{
	...
	vKeyLines.clear();
	cvKeyLineDescriptors.release();

	m_pLSDDescriptors->detect(cvColorImage, vKeyLines, cvMaskImage);

	if (vKeyLines.empty())
		return;
	m_pLSDDescriptors->compute(cvColorImage, vKeyLines, cvKeyLineDescriptors);
}
...
void	CLineSegmentWrapper::FindCoarseMatch(std::vector<cv::line_descriptor::KeyLine>& vSrcKeyLines, 
											 ...
											 const std::vector<SiftKeyPoint>& vSrcKeyPoints, const std::vector<SiftKeyPoint>& vDstKeyPoints,
											 std::vector<std::pair<int, int>>& vKeyPointMatchPair)
{
	...
	for (int i = 0; i < (int)vCoarseMatches.size(); i++) {
		...
		if (2 * nMatchingScore > std::min((int)vSrcKeyPointsIdxInKeyLine.size(), (int)vDstKeyPointsIdxInKeyLine.size()))
...
std::vector<int> CLineSegmentWrapper::GetBoundedKeyPoints(const cv::line_descriptor::KeyLine& keyLine, const std::vector<SiftKeyPoint>& vKeyPoints)
{
	...
	double length1 = keyLine.lineLength*2;
...
void	CLineSegmentWrapper::FindCoarseMatch(cv::Mat cvSrcKeyLineDescriptors, cv::Mat cvDstKeyLineDescriptors, std::vector<std::pair<int, int>>& vKeyLineCoarseMatchPair)
{
	std::map<int, int> mKeyPointMatchPair;
	std::vector<cv::DMatch> vCoarseMatches;
	...
	for (int i = 0; i < (int)vCoarseMatches.size(); i++)
		vKeyLineCoarseMatchPair.push_back(std::pair<int, int>(vCoarseMatches[i].queryIdx, vCoarseMatches[i].trainIdx));
}