...
namespace GoogleARCore.Examples.ComputerVision
{
    ...
    public class ComputerVisionController : MonoBehaviour
    {
        ...
        // YSKWAK-S
        private static int iRawImageCount = 0;
        //bool bDev = true;
        // YSKWAK
        ...
        public void Update()
        {
            ...
            try
            {
                byte[] s_ImageBuffer = new byte[0];
                //byte[] s_ImageBufferU = new byte[0];
                //byte[] s_ImageBufferV = new byte[0];

                int s_ImageBufferSize = 0;
                //int s_ImageBufferSizeU = 0;
                //int s_ImageBufferSizeV = 0;
                ...
                using (var image = Frame.CameraImage.AcquireCameraImageBytes())
                {
                    ...
                    //int bufferSizeU = image.UVRowStride * image.Height;
                    //if (bufferSizeU != s_ImageBufferSizeU || s_ImageBufferU.Length == 0)
                    //{
                    //    s_ImageBufferSizeU = bufferSizeU;
                    //    s_ImageBufferU = new byte[bufferSizeU];
                    //}

                    //int bufferSizeV = image.UVRowStride * image.Height;
                    //if (bufferSizeV != s_ImageBufferSizeV || s_ImageBufferV.Length == 0)
                    //{
                    //    s_ImageBufferSizeV = bufferSizeV;
                    //    s_ImageBufferV = new byte[bufferSizeV];
                    //}

                    // Move raw data into managed buffer.
                    System.Runtime.InteropServices.Marshal.Copy(image.Y, s_ImageBuffer, 0, bufferSize);
                    //System.Runtime.InteropServices.Marshal.Copy(image.U, s_ImageBufferU, 0, bufferSizeU);
                    //System.Runtime.InteropServices.Marshal.Copy(image.V, s_ImageBufferV, 0, bufferSizeV);

                    System.IO.StreamWriter sw = new System.IO.StreamWriter(Application.persistentDataPath + "/Fine.txt");

                    //Texture2D tex = new Texture2D(image.Width, image.Height);

                    //for (int y = 0; y < image.Height; y++)
                    //{
                    //    for (int x = 0; x < image.YRowStride; x++)
                    //    {
                    //        Color color = new Color(s_ImageBuffer[y * image.Height + x], s_ImageBuffer[y * image.Height + x], s_ImageBuffer[y * image.Height + x]);
                    ...
                    //System.IO.File.WriteAllBytes(Application.persistentDataPath + "/Recording/RawImage/" + iRawImageCount + ".jpg", bytes);

                    iRawImageCount++;

                    //tex.SetPixels(s_ImageBuffer);
                    //tex.SetPixels()

                    sw.WriteLine(s_ImageBuffer + ", " + bufferSize.ToString() );
                    sw.Flush();
                    sw.Close();
            ...
            {
                System.IO.StreamWriter sw = new System.IO.StreamWriter(Application.persistentDataPath + "/test.txt");
                sw.WriteLine(e);
                sw.Flush();
                sw.Close();
            }
